...
class BME280:
    ...
    def __init__(self,
        ...
        dig_88_a1 = self.i2c.mem_read(26, self.address, 0x88)
        dig_e1_e7 = self.i2c.mem_read(7, self.address, 0xE1)
        ...
        self.i2c.mem_write(self._l1_barray, self.address, BME280_REGISTER_CONTROL)
        self.t_fine = 0
    ...
    def read_raw_data(self, result):
        ...
        self.i2c.mem_write(self._l1_barray, self.address, BME280_REGISTER_CONTROL_HUM)
        self._l1_barray[0] = self._mode << 5 | self._mode << 2 | MODE_FORCED
        self.i2c.mem_write(self._l1_barray, self.address, BME280_REGISTER_CONTROL)

        # Wait for conversion to complete
        while self.i2c.mem_read(1, self.address, BME280_REGISTER_STATUS)[0] & 0x08:
            time.sleep_ms(5)

        # burst readout from 0xF7 to 0xFE, recommended by datasheet
        self.i2c.mem_read(self._l8_barray, self.address, 0xF7 )
    ...
    def get_data(self):
    # first reading is often not settled, so loop several values before returning value
        temp = 0
        pres = 0
        hum = 0
        for x in range(0, 3):
            temp, pres, hum = self.read_compensated_data()
...
def calibrate():
    # Collect data for results calibration
    from pyb import I2C
    from CSV_Util import CSV_Util
    t_header = 'Degree_C'
    p_header = 'hPa'
    ...
    # files to use
    t_dir = '/home/pi/python/PyRock/data/temp.csv'
    p_dir = '/home/pi/python/PyRock/data/pressure.csv'
    h_dir = '/home/pi/python/PyRock/data/humidity.csv'
    ...
    while True:
       ...
       print("{:.2f}hPa".format(p/100))
       print("{:.2f}%".format(h))
       t_file.write(round(t, 2))
       p_file.write(round(p, 2))
       h_file.write(round(h, 2))
       time.sleep(2)
